...
function InsertyMySQL(initiate,participants,start_time,end_time,duration,voice_url){
  ...
  connection.connect((err) => {
      ...
      // 要执行的 SQL 查询，这里是一个更新操作示例
      const updateSql = 'insert into sys_voice (initiate,participants,start_time,end_time,duration,voice_url) values(?,?,?,?,?,?);';
      const values = [initiate,participants,start_time,end_time,duration,voice_url]; // 替换为实际的值
